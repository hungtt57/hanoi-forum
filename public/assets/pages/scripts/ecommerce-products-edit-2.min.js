var EcommerceProductsEdit = function () {
    var e = function () {
        var e = new plupload.Uploader({
            ...
            url: "/admin/course/upload-document-course-update/" + course_id,
            filters: {
                max_file_size: "10mb",
            ...
            init: {
                PostInit: function () {
                    ...
                    if (t.result && "OK" == t.result) {
                        $('#form-name-file').append("<input type='hidden' name='file-id[]' value='" + t.id + "' />")
                        $("#uploaded_file_" + a.id + " > .status").removeClass("label-info").addClass("label-success").html('<i class="fa fa-check"></i> Thành công')

                        $.ajax({
                            url: '/admin/course/get-document',
                            type: 'get',
                            data: {
                                course_id: course_id
                            },
                            ...
                            success: function (response) {
                                $('#list-documents').html(response);

                            }
                        });

                    } else $("#uploaded_file_" + a.id + " > .status").removeClass("label-info").addClass("label-danger").html('<i class="fa fa-warning"></i> Lỗi'), App.alert({
                        type: "danger",
                    ...
                    App.alert({type: "danger", message: a.message, closeInSeconds: 10, icon: "warning"})
                }
            }
        });
        e.init();
        e.bind('StateChanged', function () {
            if (e.files.length === (e.total.uploaded + e.total.failed)) {
                var data = $('#form-name-file').serialize();
                $.ajax({
                    url: '/admin/course/update-name-file',
                    data: data,
                    type: 'post',
                    success: function () {
                        $.ajax({
        ...
        e.init({
            ...
            dataTable: {
                ...
                pageLength: 10,
                ajax: {url: "../demo/ecommerce_product_reviews.php"},
...
//             $.ajax({
//                 url: '/admin/course/create-course-attr',
//                 type: 'post',
//                 dataType: 'json',
//                 success: function (response) {
